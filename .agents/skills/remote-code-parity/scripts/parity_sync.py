...
def derive_workspace_id(repo_root: Path) -> str:
    base = (
        WORKSPACE_ID_PATTERN.sub("-", repo_root.name.lower()).strip(".-") or "workspace"
    )
...
def resolve_machine_record(
    inventory: dict[str, Any], identifier: str
) -> dict[str, Any]:
    matches = []
    for record in inventory.get("machines", []):
        if (
            record.get("alias") == identifier
            or record.get("host", {}).get("ip") == identifier
        ):
...
def build_derived_args(repo_root: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    runtime_root = (
        args.runtime_root
        or record.get("container", {}).get("workdir")
        or "/vllm-workspace"
    )
    ...
    if not container_name:
        raise RuntimeError(
            f"machine {args.machine!r} is missing container.name in inventory"
        )
    container_port = record.get("container", {}).get("ssh_port")
    if not isinstance(container_port, int):
        raise RuntimeError(
            f"machine {args.machine!r} is missing container.ssh_port in inventory"
        )
    ...
    return {
        ...
        "inventory_path": str(
            canonical_inventory_path(repo_root)
            if canonical_inventory_path(repo_root).exists()
            else legacy_inventory_path(repo_root)
        ),
    }


def build_derived_args_from_session(
    ...
    runtime_root = (
        args.runtime_root
        or container.get("runtime_root")
        or container.get("workdir")
        or "/vllm-workspace"
    )
    workspace_id = (
        args.workspace_id or session.get("workspace_id") or session["session_id"]
    )
    container_name = container.get("name")
    if not container_name:
        raise RuntimeError(
            f"session {session['session_id']!r} is missing remote.container.name"
        )
    container_port = container.get("ssh_port")
    if not isinstance(container_port, int):
        raise RuntimeError(
            f"session {session['session_id']!r} is missing remote.container.ssh_port"
        )
...
def build_low_level_command(
    derived: dict[str, Any], args: argparse.Namespace
) -> list[str]:
    ...
    cmd = [
        ...
        "--container-port",
        str(derived["container_port"]),
        "--container-user",
        derived["container_user"],
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Resolve a managed machine from inventory and run container-only remote-code-parity sync.",
        allow_abbrev=False,
    )
...
def main() -> int:
    ...
    if not args.force_reinstall:
        ...
        mode = resolve_sync_mode(
            consent_state, derived["server_name"], derived["container_identity"]
        )
        if mode == "unset":
            print(
                json_dump(
