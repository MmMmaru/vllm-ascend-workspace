...
def set_decision(
    ...
    *,
    ...
    if not approved_by_user:
        raise RuntimeError(
            "explicit --approved-by-user is required before writing first-install consent"
        )
    container = (
        state.setdefault("consents", {})
        .setdefault(server_name, {})
        .setdefault("containers", {})
        .setdefault(container_identity, {})
    )
    container.update(
        {
            "decision": decision,
...
def resolve_sync_mode(
    state: dict[str, Any], server_name: str, container_identity: str
) -> str:
    return (
        state.get("consents", {})
        .get(server_name, {})
...
def set_sync_mode(
    ...
    *,
    ...
    if not approved_by_user:
        raise RuntimeError(
            "explicit --approved-by-user is required before writing sync-mode"
        )
...
def run_set(args: argparse.Namespace) -> int:
    ...
    _, path, _ = update_state(
        repo_root, FILENAME, {"schema_version": 1, "consents": {}}, apply_update
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Manage first-install consent for remote-code-parity.",
        allow_abbrev=False,
    )
