...
def resolved_root_preserve_paths(
    marker_dirname: str, extra_paths: list[str]
) -> tuple[str, ...]:
    ordered: list[str] = []
...
def discover_repo_tree(
    repo: Path, relpath: str = ".", submodule_name: str | None = None
) -> RepoNode:
    ...
    for entry in list_submodules(repo):
        ...
        child_relpath = (
            entry.path if relpath in ("", ".") else f"{relpath}/{entry.path}"
        )
...
def commit_message(workspace_id: str, relpath: str) -> str:
    return (
        f"remote-code-parity tree snapshot {workspace_id} {sanitize_repo_id(relpath)}"
    )
...
def build_synthetic_snapshot(
    ...
    *,
    ...
    try:
        ...
        for child in node.children:
            ...
            git(
                ...
                [
                    ...
                    "--add",
                    "--cacheinfo",
                    f"160000,{child_record.commit},{child_rel_to_repo}",
                ],
                env=env,
            )
        ...
        commit = git(
            repo,
            ["commit-tree", tree, "-m", commit_message(workspace_id, node.relpath)],
            env=env,
        ).stdout.strip()
        if source_head:
            diff = git(
                repo, ["diff", "--name-only", f"{source_head}..{commit}"]
            ).stdout.splitlines()
        else:
            diff = git(
                repo, ["show", "--pretty=", "--name-only", commit]
            ).stdout.splitlines()
...
def cleanup_synthetic_refs(workspace_root: Path, records: list[SnapshotRecord]) -> None:
    for record in records:
        repo = (
            ...
            else workspace_root / record.relpath
        )
...
def load_consent(repo_root: Path) -> dict[str, Any]:
    return load_state(
        repo_root, CONSENT_FILENAME, {"schema_version": 1, "consents": {}}
    )


def resolve_install_consent(
    repo_root: Path, server_name: str, container_identity: str
) -> str:
...
def mirror_path_for(
    container_cache_root: str, workspace_id: str, record: SnapshotRecord
) -> str:
    root = remote_path(
        cache_workspace_root(container_cache_root, workspace_id), "mirrors"
    )
...
def bundle_path_for(
    ...
    root = remote_path(
        cache_workspace_root(container_cache_root, workspace_id), "bundles"
    )
    return remote_path(root, f"{record.repo_id}-{record.commit}.bundle")


def manifest_path_for(
    container_cache_root: str, workspace_id: str, snapshot_id: str
) -> str:
    return remote_path(
        cache_workspace_root(container_cache_root, workspace_id),
        "manifests",
        f"{snapshot_id}.json",
    )
...
def lock_path_for(
    container_cache_root: str, workspace_id: str, container_identity: str
) -> str:
    token = (
        re.sub(r"[^A-Za-z0-9._-]+", "-", container_identity).strip(".-") or "container"
    )
    return remote_path(
        cache_workspace_root(container_cache_root, workspace_id), "locks", token
    )
...
def ensure_remote_bare_repos(
    container: SshEndpoint, mirror_paths: list[str], dry_run: bool
) -> None:
...
def push_snapshot_to_mirror(
    ...
    *,
    ...
    local_bundle = tempfile.NamedTemporaryFile(
        prefix="parity-bundle-", suffix=".bundle", delete=False
    )
    local_bundle.close()
    ...
    try:
        git(
            repo,
            ["bundle", "create", str(local_bundle_path), record.ref],
            timeout=DEFAULT_GIT_TRANSPORT_TIMEOUT_SECONDS,
        )
        ssh_stream_bytes_to_file(
            container, remote_bundle_path, local_bundle_path.read_bytes()
        )
        script = "\n".join(
            [
                "set -eo pipefail",
...
def acquire_container_lock(
    ...
    *,
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"could not acquire container lock {lock_path}: {result.stderr or result.stdout}"
        )


def release_container_lock(
    container: SshEndpoint, lock_path: str, dry_run: bool
) -> None:
    if dry_run:
        return
    ssh_exec(
        container, f"rm -rf {quoted(lock_path)} >/dev/null 2>&1 || true", check=False
    )


def upload_manifest(
    container: SshEndpoint, manifest_path: str, manifest: dict[str, Any], dry_run: bool
) -> None:
    if dry_run:
        return
...
def materialize_runtime(
    *,
    ...
    def render_repo_step(record: SnapshotRecord) -> str:
        ...
        lines = [
            "set -eo pipefail",
            f"mkdir -p {quoted(PurePosixPath(repo_dir).parent.as_posix())}",
        ]
        if record.relpath in ("", "."):
            lines.append(
                f"if [ ! -e {quoted(remote_path(repo_dir, '.git'))} ]; then git init {quoted(repo_dir)} >/dev/null; fi"
            )
        else:
            lines.append(
        ...
        for child in record.submodules:
            child_relpath = (
                child["path"]
                if record.relpath in ("", ".")
                else f"{record.relpath}/{child['path']}"
            )
            child_record = record_by_relpath[child_relpath]
            child_mirror = mirror_path_for(
                container_cache_root, workspace_id, child_record
            )
...
def reinstall_required_for_repo(
    record: SnapshotRecord, patterns: tuple[str, ...]
) -> bool:
...
def dependency_install_required_for_repo(record: SnapshotRecord) -> bool:
    return any(
        glob_match_any(path, DEPENDENCY_INSTALL_PATTERNS)
    ...
    *,
    ...
    if step == "uninstall":
        pkg_args = (
            ...
            else "vllm vllm-ascend vllm_ascend"
        )
...
def read_runtime_install_marker(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            f"container runtime-install marker at {path} is invalid JSON: {exc}"
        ) from exc


def first_install_needed(
    marker: RuntimeInstallMarker, container_identity: str, runtime_root: str
) -> bool:
    if marker.record is None:
        return True
    return (
        marker.record.get("container_identity") != container_identity
        or marker.record.get("runtime_root") != runtime_root
    )
...
def verify_runtime_commits(
    *,
    ...
    for relpath in expected:
        repo_dir = (
            runtime_root if relpath in ("", ".") else remote_path(runtime_root, relpath)
        )
        lines.append(
    ...
    def apply_update(state: dict[str, Any]) -> None:
        ...
        containers[container_identity] = {
            ...
            "last_snapshot_commits": {
                record.relpath: record.commit for record in records
            },
            "last_head_commits": {
                record.relpath: record.source_head for record in records
            },
            "last_runtime_install_env": runtime_install_env or {},
        }

    update_state(
        repo_root, STATE_FILENAME, {"schema_version": 2, "servers": {}}, apply_update
    )
...
def build_snapshot_records(
    workspace_root: Path, workspace_id: str, snapshot_id: str, denylist: tuple[str, ...]
) -> list[SnapshotRecord]:
...
def final_manifest(
    ...
    *,
    status: str,
    reinstall_status: str,
    runtime_commits: dict[str, str] | None,
) -> dict[str, Any]:
...
def run_plan(args: argparse.Namespace) -> int:
    ...
    container_cache_root = validate_absolute_posix_path(
        args.container_cache_root, label="container cache root"
    )
    marker_dirname = validate_relative_posix_path(
        args.marker_dirname, label="marker dirname"
    )
    root_preserve_paths = resolved_root_preserve_paths(
        marker_dirname, args.preserve_path
    )
    snapshot_id = args.snapshot_id or now_utc().replace(":", "").replace("-", "")
    records = build_snapshot_records(
        workspace_root, workspace_id, snapshot_id, tuple(DEFAULT_DENYLIST)
    )
    try:
...
def run_sync(args: argparse.Namespace) -> int:
    ...
    snapshot_id = (
        args.snapshot_id
        or now_utc().replace(":", "").replace("-", "") + "-" + uuid.uuid4().hex[:8]
    )
    container = SshEndpoint(
        host=args.container_host, port=args.container_port, user=args.container_user
    )
    ...
    try:
        try:
            ...
            reinstall_vllm = (
                reinstall_required_for_repo(record_map["vllm"], VLLM_REINSTALL_PATTERNS)
                if "vllm" in record_map
                else False
            )
            reinstall_vllm_ascend = (
                reinstall_required_for_repo(
                    record_map["vllm-ascend"], VLLM_ASCEND_REINSTALL_PATTERNS
                )
                if "vllm-ascend" in record_map
                else False
            )
            vllm_dependency_changed = (
                dependency_install_required_for_repo(record_map["vllm"])
                if "vllm" in record_map
                else False
            )
            vllm_ascend_dependency_changed = (
                dependency_install_required_for_repo(record_map["vllm-ascend"])
                if "vllm-ascend" in record_map
                else False
            )
            ...
            last_container_state = (
                prior_runtime_state.get("servers", {})
            ...
            if (
                "vllm" in record_map
                and last_head_commits.get("vllm")
                and record_map["vllm"].source_head != last_head_commits["vllm"]
            ):
                vllm_head_drift = True
                reinstall_vllm = True
            if (
                "vllm-ascend" in record_map
                and last_head_commits.get("vllm-ascend")
                and record_map["vllm-ascend"].source_head
                != last_head_commits["vllm-ascend"]
            ):
            ...
            install_vllm_ascend_deps = (
                vllm_ascend_dependency_changed or vllm_ascend_head_drift
            )
            ...
            if args.apply_mode in {"source-only", "materialize"}:
                ...
                lock_path = lock_path_for(
                    container_cache_root, workspace_id, args.container_identity
                )
                current_phase = "acquire-lock"
                emit_progress(
                    current_phase, lock_path=lock_path, apply_mode=args.apply_mode
                )
                ...
                try:
                    ...
                    all_mirror_paths = [
                        mirror_path_for(container_cache_root, workspace_id, r)
                        for r in records
                    ]
                    ...
                    for record in records:
                        ...
                        push_snapshot_to_mirror(
                            ...
                            container=container,
                            mirror_path=mirror_path_for(
                                container_cache_root, workspace_id, record
                            ),
                            container_cache_root=container_cache_root,
                            record=record,
                            workspace_id=workspace_id,
                    ...
                    if args.apply_mode == "materialize":
                        ...
                        emit_progress(
                            current_phase, runtime_root=runtime_root, install="skipped"
                        )
                        materialize_runtime(
                            container=container,
                            runtime_root=runtime_root,
            ...
            if (
                ...
                if observed == snapshot_commits:
                    ...
                    summary = summary_payload(
                        ...
                        reason=None,
                        first_install=False,
                        runtime_install_env=last_container_state.get(
            ...
            first_install = first_install_needed(
                marker, args.container_identity, runtime_root
            )
            if first_install:
                current_phase = "check-consent"
                emit_progress(current_phase, container_identity=args.container_identity)
                consent = resolve_install_consent(
                    workspace_root, args.server_name, args.container_identity
                )
                if consent != "allow":
                    summary = summary_payload(
                        status="blocked",
                ...
                reinstall_vllm_ascend = (
                    True if "vllm-ascend" in record_map else reinstall_vllm_ascend
                )
                install_vllm_ascend_deps = (
                    True if "vllm-ascend" in record_map else install_vllm_ascend_deps
                )
            ...
            try:
                ...
                if reinstall_vllm or reinstall_vllm_ascend:
                    ...
                    if not first_install:
                        ...
                        emit_progress(
                            "runtime-install-uninstall", packages=uninstall_pkgs
                        )
                    ...
                    if reinstall_vllm_ascend:
                        ...
                        emit_progress(
                            "runtime-install-vllm-ascend", package="vllm-ascend"
                        )
                        run_runtime_install_step(
                            container=container,
                            runtime_root=runtime_root,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Prepare or enforce remote code parity for a ready runtime.",
        allow_abbrev=False,
    )
    ...
    def add_shared_arguments(target: argparse.ArgumentParser) -> None:
        target.add_argument(
            "--workspace-root", required=True, help="Local workspace root."
        )
        target.add_argument(
            "--workspace-id",
            required=True,
            help="Stable workspace id used for container cache namespacing.",
        )
        ...
        target.add_argument(
            "--container-cache-root", default=DEFAULT_CONTAINER_CACHE_ROOT
        )
    ...
    plan = subparsers.add_parser(
        "plan", help="Build a synthetic snapshot manifest without remote mutations."
    )
    ...
    sync = subparsers.add_parser(
        "sync",
        help="Publish container-local mirrors, materialize runtime state, and reinstall when required.",
    )
    ...
    sync.add_argument(
        "--force-reinstall",
        action="store_true",
        help="Force reinstall of vllm and vllm-ascend regardless of what changed.",
    )
