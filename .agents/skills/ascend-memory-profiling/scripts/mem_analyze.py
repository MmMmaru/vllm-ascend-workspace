...
def parse_npu_smi_hbm(data: dict[str, dict]) -> dict[int, dict]:
    ...
    for dev_str, info in data.items():
        dev_id = (
            int(dev_str) if isinstance(dev_str, str) and dev_str.isdigit() else dev_str
        )
        result[dev_id] = info
    return result
...
_COMPILE_WARMUP_RE = re.compile(
    r"torch\.compile and initial profiling/warmup run together took ([\d.]+) s"
)
_ENCODER_CACHE_RE = re.compile(
    r"Encoder cache will be initialized with a budget of (\d+) tokens"
)
    ...
    with open(csv_path) as f:
        ...
        for row in reader:
            ...
            events.setdefault(event, []).append(
                {"timestamp_us": ts_val, "hbm_kb": hbm_val}
            )
    return events
...
def format_weight_breakdown(weight_info: dict, vllm_gb: float = 0) -> list[str]:
    ...
    lines.append(
        f"  总权重(文件): {weight_info['total_gib']:.4f} GiB | "
        f"每设备(理论分片): {per_dev_gib:.4f} GiB ({per_dev_gb:.4f} GB)"
    )
    lines.append(
        f"  分片策略: TP={weight_info['tp']}, DP={weight_info['dp']}, "
        f"EP={weight_info['ep']}"
    )
    if vllm_gb:
        ...
        lines.append(
            f"  差值(safetensors vs vLLM): {sign}{diff_gib:.4f} GiB ({diff_pct:.1f}%)"
        )
        if diff_gib > 0.1:
            lines.append("  说明: safetensors 高于 vLLM 实际值，可能原因:")
            lines.append(
                "    - MTP 模块复用 base model 的 embed_tokens/lm_head (共享内存)"
            )
            lines.append("    - 部分 F32 参数在加载时被转为 BF16")
            lines.append("    - vision encoder 在纯文本推理下可能未加载")
    lines.append("")
    lines.append(
        f"  {'组件':<28} | {'总量 (GiB)':>10} | {'每设备 (GiB)':>12} | {'占比':>6} | dtypes"
    )
    lines.append("  " + "-" * 90)
...
def estimate_weight_size(config: dict, tp: int, dp: int = 1) -> dict[str, Any]:
    ...
    if num_experts and num_experts > 1:
        ...
        shared_params_per_layer = (
            3 * hidden * shared_expert_intermediate if shared_expert_intermediate else 0
        )
        gate_params_per_layer = hidden * num_experts
        ff_params_per_layer = (
            expert_params_per_layer + shared_params_per_layer + gate_params_per_layer
        )
    else:
    ...
    if num_experts and num_experts > 1 and tp > 1:
        ...
        expert_bytes = (
            ...
            * 3
            * hidden
            * (moe_intermediate or intermediate)
            * bpe
            / ep
        )
        shared_bytes = (
            num_layers * 3 * hidden * shared_expert_intermediate * bpe / tp
...
def generate_device_report(
    ...
    other_msprof = sum(
        v
        for k, v in msprof_components.items()
    ...
    if weights_gb:
        ...
        weight_evidence = (
            f"vllm_serve.log: Loading model weights took {weights_gb:.4f} GB"
        )
    elif precise_gib:
    ...
    if precise_gib and weights_gb:
        diff_pct = (
            abs(weights_gb - precise_gib) / precise_gib * 100 if precise_gib else 0
        )
        ...
        weight_corr.append(
            f"config 理论值: {theory_gib:.2f} GiB / 误差: {diff_pct:.1f}%"
        )
    ...
    report.cross_validation = {
        ...
        "unattributed_pct": round(abs(unattributed) / used_mb * 100, 1)
        if used_mb
        else 0,
    }
    if app_mb > 0:
...
def format_report_text(
    ...
    lines.append(
        f"采集模式: {'attach (挂载已有服务)' if mode == 'attach' else 'standalone (独立管理服务)'}"
    )
    ...
    if mode == "attach":
        ...
        if baseline_src == "unavailable":
            lines.append(
                "基线数据: 不可用 (attach 模式，建议通过 --baseline-from 复用历史基线)"
            )
        else:
    ...
    for r in reports:
        lines.append(
            f"[Device {r.device_id}] 总 HBM: {r.total_hbm_mb / 1024:.2f} GiB | "
            f"已用: {r.used_hbm_mb / 1024:.2f} GiB"
        )
        lines.append("")
        lines.append(
            f"{'组件':<28} | {'占用 (MB)':>10} | {'占用 (GiB)':>10} | {'占比':>6} | 主数据源"
        )
        lines.append("-" * 110)
        ...
        lines.append(
            f"  组件加总:              {cv.get('component_sum_mb', 0):,.1f} MB"
        )
        lines.append(
            f"  未归因:                {cv.get('unattributed_mb', 0):,.1f} MB "
            f"({cv.get('unattributed_pct', 0):.1f}%)"
        )
        if "msprof_app_mb" in cv:
            lines.append(f"  msprof APP:            {cv['msprof_app_mb']:,.1f} MB")
            lines.append(
                f"  vLLM 权重+KV:          {cv.get('vllm_weight_plus_kv_mb', 0):,.1f} MB"
            )
            lines.append(
                f"  APP - (权重+KV):       {cv.get('app_minus_weight_kv_mb', 0):,.1f} MB "
                "(含激活/内部缓冲)"
            )
        lines.append("")
...
def find_all_msprof_csvs(run_dir: Path, pattern: str) -> list[Path]:
    ...
    if not csv_dir.exists():
        return []
    return sorted(
        f for f in csv_dir.iterdir() if pattern in f.name and f.suffix == ".csv"
    )
...
def main() -> None:
    ...
    has_experts = bool(
        mc.get("num_experts", 0) or mc.get("text_config", {}).get("num_experts", 0)
    )
    ep_flag = manifest.get("enable_expert_parallel")
    enable_ep = ep_flag if ep_flag is not None else has_experts
    weight_precise = (
        compute_weight_from_manifest(weight_manifest, tp, dp, enable_ep)
        if weight_manifest
        else {}
    )
    ...
    for dev_id in devices:
        ...
        report = generate_device_report(
            ...
            tp=tp,
            msprof_scope="per-device"
            if dev_id in per_device_msprof
            else "process-level",
        )
    ...
    json_report = {
        ...
        "msprof_per_device": {
            str(d): {k: round(v, 2) for k, v in comps.items()}
            for d, comps in per_device_msprof.items()
        },
        "weight_theory": weight_theory,
        "weight_precise": weight_precise,
