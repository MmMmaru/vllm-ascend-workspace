...
def add_endpoint_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--runtime-env", dest="runtime_env", action="store_true", default=None
    )
...
def print_payload(payload: dict[str, Any]) -> int:
    ...
    return (
        0 if payload.get("result", {}).get("outcome") in {"success", "cancelled"} else 1
    )


def error_payload(
    tool: str, *, outcome: str, status: str, error: str
) -> dict[str, Any]:
    result = make_result(
        tool=f"remote.{tool}",
...
def build_parser(tool: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--input-json",
        help="Read complete tool arguments from a JSON file, or '-' for stdin.",
    )
    ...
    if tool == "bash":
        ...
        if tool == "job_tail":
            ...
            parser.add_argument(
                "--stream", default="both", choices=["stdout", "stderr", "both"]
            )
        if tool == "job_stop":
...
def run_tool(tool: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    endpoint = None
    if tool not in {"job_status", "job_tail", "job_stop"} or any(
        data.get(k)
        for k in ("host", "port", "alias", "session_id", "session_file", "machine")
    ):
    ...
    if tool == "read":
        ...
        return remote_read(
            ...
            offset=int(data.get("offset", args.offset)),
            limit=int(data.get("limit", args.limit)),
            allow_symlink=bool(data.get("allow_symlink", args.allow_symlink)),
            client_context_id=data.get("client_context_id") or args.client_context_id,
            timeout_ms=timeout_ms,
        )
    if tool == "ls":
        assert endpoint is not None
        return remote_ls(
            endpoint,
            path=data.get("path") or args.path,
            limit=int(data.get("limit", args.limit)),
            all=bool(data.get("all", args.all)),
            timeout_ms=timeout_ms,
        )
    if tool == "write":
        assert endpoint is not None
        ...
        return remote_write(
            ...
            overwrite=bool(data.get("overwrite", args.overwrite)),
            create_dirs=bool(data.get("create_dirs", args.create_dirs)),
            client_context_id=data.get("client_context_id") or args.client_context_id,
            timeout_ms=timeout_ms,
        )
    if tool == "edit":
        ...
        return remote_edit(
            ...
            else args.old_string,
            new_string=data.get("new_string")
            if data.get("new_string") is not None
            else args.new_string,
            replace_all=bool(data.get("replace_all", args.replace_all)),
            client_context_id=data.get("client_context_id") or args.client_context_id,
            timeout_ms=timeout_ms,
        )
    if tool == "multi_edit":
    ...
    if tool == "glob":
        ...
        return remote_glob(
            endpoint,
            pattern=data.get("pattern") or args.pattern or "*",
            path=data.get("path") or args.path,
            limit=int(data.get("limit", args.limit)),
            respect_gitignore=bool(
                data.get("respect_gitignore", args.respect_gitignore)
            ),
            timeout_ms=timeout_ms,
        )
    if tool == "grep":
        ...
        return remote_grep(
            ...
            pattern=data.get("pattern") or args.pattern or "",
            path=data.get("path") or args.path,
            glob=data.get("glob") or args.glob,
            type=data.get("type") or args.type,
            output_mode=data.get("output_mode") or args.output_mode,
            multiline=bool(data.get("multiline", args.multiline)),
            limit=int(data.get("limit", args.limit)),
            timeout_ms=timeout_ms,
        )
    if tool == "apply_patch":
    ...
    if tool == "job_tail":
        return remote_job_tail(
            endpoint,
            job_id=data.get("job_id") or args.job_id,
            lines=int(data.get("lines", args.lines)),
            stream=data.get("stream") or args.stream,
        )
    if tool == "job_stop":
        return remote_job_stop(
            endpoint,
            job_id=data.get("job_id") or args.job_id,
            force=bool(data.get("force", args.force)),
        )
    if tool == "artifact_manifest":
        assert endpoint is not None
        return remote_artifact_manifest(
            endpoint,
            remote_path=data.get("remote_path") or args.remote_path,
            timeout_ms=timeout_ms,
        )
    if tool == "artifact_pull":
        assert endpoint is not None
        return remote_artifact_pull(
            endpoint,
            remote_path=data.get("remote_path") or args.remote_path,
            local_dir=data.get("local_dir") or args.local_dir,
            timeout_ms=timeout_ms,
        )
    if tool == "artifact_push":
        assert endpoint is not None
        return remote_artifact_push(
            endpoint,
            local_path=data.get("local_path") or args.local_path,
            remote_path=data.get("remote_path") or args.remote_path,
            timeout_ms=timeout_ms,
        )
    if tool == "context_snapshot":
        assert endpoint is not None
        return remote_context_snapshot(
            endpoint,
            timeout_ms=timeout_ms,
            live_probe=not bool(data.get("no_live_probe", args.no_live_probe)),
        )
    if tool == "probe":
        assert endpoint is not None
...
def main(tool: str) -> int:
    ...
    try:
        ...
        return print_payload(
            error_payload(
                tool, outcome="needs_input", status="endpoint_required", error=str(exc)
            )
        )
    except FileNotFoundError as exc:
        return print_payload(
            error_payload(
                tool, outcome="needs_input", status="not_found", error=str(exc)
            )
        )
    except ValueError as exc:
        return print_payload(
            error_payload(
                tool, outcome="needs_input", status="invalid_input", error=str(exc)
            )
        )
