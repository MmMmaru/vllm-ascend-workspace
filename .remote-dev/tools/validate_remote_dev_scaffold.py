...
def mcp_and_burden_checks() -> dict[str, Any]:
    ...
    expected_scripts = {
        REMOTE_DEV_ROOT / "tools" / (name.replace(".", "_") + ".py")
        for name in TOOL_SCHEMAS
    }
    endpoint_fields = set(ENDPOINT_PROPS)
    required_by_tool = {
        name: set(schema.get("required", [])) for name, schema in TOOL_SCHEMAS.items()
    }
    endpoint_required = {
    ...
    endpoint_selector_missing = sorted(
        set(TOOL_SCHEMAS)
        - {"remote.job_status", "remote.job_tail", "remote.job_stop"}
        - endpoint_selector_tools
    )
    own_required_counts = {
    ...
    if endpoint_required:
        failures.append(
            "endpoint fields should not be top-level required by tool schemas"
        )
    if endpoint_selector_missing:
        failures.append(
            "remote tools should express endpoint selector anyOf in tool schemas"
        )
    ...
    return {
        ...
        "max_own_required_fields": max(own_required_counts.values())
        if own_required_counts
        else 0,
        "failures": failures,
    }
...
def require_outcome(
    ...
    *,
    ...
    if outcome not in outcomes or (statuses is not None and status not in statuses):
        raise RuntimeError(
            f"{name} returned outcome={outcome} status={status}: {payload.get('text', '')[:1000]}"
        )
    return {
        "name": name,
        "outcome": outcome,
...
def run_parallel_worker(
    endpoint: dict[str, Any], scratch: str, index: int, timeout_ms: int
) -> dict[str, Any]:
    ...
    checks = [
        require_outcome(
            f"parallel_{index}_create",
            call_tool(
                "remote.bash",
                {**endpoint, "command": command, "timeout_ms": timeout_ms},
            ),
        ),
        require_outcome(
            f"parallel_{index}_read",
            call_tool(
                "remote.read",
                {**endpoint, "file_path": file_path, "timeout_ms": timeout_ms},
            ),
            statuses={"ok"},
        ),
        require_outcome(
...
def live_endpoint_checks(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not any(
        endpoint.get(key)
        for key in ("host", "alias", "session_id", "session_file", "machine")
    ):
    ...
    try:
        ...
        checks.append(
            require_outcome(
                "probe",
                call_tool("remote.probe", {**endpoint, "timeout_ms": timeout_ms}),
            )
        )
        checks.append(
            require_outcome(
                ...
                call_tool(
                    ...
                    {**endpoint, "timeout_ms": timeout_ms, "live_probe": True},
                ),
            )
        )
        checks.append(
        ...
        checks.append(
            require_outcome(
                ...
                call_tool(
                    ...
                    {
                        **endpoint,
                        "cwd": f"{scratch}/missing",
                        "command": "pwd",
                        "timeout_ms": timeout_ms,
                    },
                ),
                outcomes={"failed"},
                statuses={"cwd_not_found"},
            )
        )
        checks.append(
            require_outcome(
                ...
                call_tool(
                    "remote.bash",
                    {**endpoint, "command": "exit 7", "timeout_ms": timeout_ms},
                ),
                outcomes={"failed"},
                statuses={"nonzero_exit"},
            )
        )
        checks.append(
            require_outcome(
                "timeout",
                call_tool(
                    "remote.bash", {**endpoint, "command": "sleep 2", "timeout_ms": 500}
                ),
                outcomes={"timeout"},
                statuses={"timeout"},
            )
        ...
        checks.append(
            require_outcome(
                ...
                call_tool(
                    "remote.bash",
                    {**endpoint, "command": setup, "timeout_ms": timeout_ms},
                ),
            )
        )
        big = call_tool(
        ...
        checks.append(
            require_outcome(
                "ls",
                call_tool(
                    "remote.ls", {**endpoint, "path": scratch, "timeout_ms": timeout_ms}
                ),
            )
        )
        checks.append(
        ...
        checks.append(
            require_outcome(
                "directory_read_rejected",
                call_tool(
                    "remote.read",
                    {**endpoint, "file_path": scratch, "timeout_ms": timeout_ms},
                ),
                outcomes={"failed"},
                statuses={"is_directory"},
            )
        )
        checks.append(
            require_outcome(
                ...
                call_tool(
                    ...
                    {
                        **narrow_endpoint,
                        "file_path": f"{scratch}/escape-link",
                        "timeout_ms": timeout_ms,
                    },
                ),
                outcomes={"blocked"},
                statuses={"path_outside_root"},
            )
        )
        checks.append(
            require_outcome(
                ...
                call_tool(
                    ...
                    {
                        **endpoint,
                        "remote_path": f"{scratch}/escape-link",
                        "timeout_ms": timeout_ms,
                    },
                ),
                outcomes={"blocked"},
        ...
        checks.append(
            require_outcome(
                "remove_escape_symlink",
                call_tool(
                    "remote.bash",
                    {
                        **endpoint,
                        "command": f"rm -f {scratch!r}/escape-link",
                        "timeout_ms": timeout_ms,
                    },
                ),
            )
...
"""
        checks.append(
            require_outcome(
                ...
                call_tool(
                    "remote.apply_patch",
                    {**endpoint, "patch": patch, "timeout_ms": timeout_ms},
                ),
                statuses={"applied"},
            )
        )
        checks.append(
            require_outcome(
                ...
                call_tool(
                    "remote.artifact_manifest",
                    {**endpoint, "remote_path": scratch, "timeout_ms": timeout_ms},
                ),
                statuses={"ok"},
            )
        )
            ...
            checks.append(
                require_outcome(
                    ...
                    call_tool(
                        ...
                        {
                            **endpoint,
                            "remote_path": f"{scratch}/file.txt",
                            "local_dir": tmp,
                            "timeout_ms": timeout_ms,
                        },
                    ),
            ...
            checks.append(
                require_outcome(
                    ...
                    call_tool(
                        ...
                        {
                            **endpoint,
                            "local_path": str(local_push),
                            "remote_path": f"{scratch}/pushed.txt",
                            "timeout_ms": timeout_ms,
                        },
                    ),
                    statuses={"ok"},
        ...
        job_payload = call_tool(
            "remote.bash",
            {
                **endpoint,
                "command": "printf 'job-out\\n'; printf 'job-err\\n' >&2",
                "cwd": scratch,
        ...
        checks.append(
            require_outcome("background_job_start", job_payload, statuses={"running"})
        )
        job_id = job_payload["result"].get(
            "job", job_payload["result"].get("extra", {}).get("job", {})
        )["job_id"]
        time.sleep(2)
        checks.append(
            require_outcome(
                ...
                call_tool(
                    "remote.job_status",
                    {**endpoint, "job_id": job_id, "timeout_ms": timeout_ms},
                ),
                statuses={"succeeded"},
            )
        )
        ...
        stdout_uri = next(
            (uri for uri in resource_uris if uri.endswith(f"/job/{job_id}/stdout")),
            None,
        )
        if not stdout_uri:
        ...
        if "job-out" not in stdout_resource.get("text", ""):
            raise RuntimeError(
                "MCP job stdout resource did not include remote log content"
            )
        checks.append(
            {"name": "mcp_job_stdout_resource", "outcome": "success", "status": "ok"}
        )
        artifact_resource_count = len(
            [
                uri
                for uri in resource_uris
                if "/artifacts/" in uri and uri.endswith("/manifest")
            ]
        )
        ...
        checks.append(
            {
                "name": "mcp_artifact_manifest_resource",
                "outcome": "success",
                "status": "ok",
                "count": artifact_resource_count,
            }
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = [
                executor.submit(
                    run_parallel_worker, endpoint, scratch, index, timeout_ms
                )
                for index in range(args.parallel_workers)
            ]
        ...
        checks.append(
            {
                ...
                "outcome": "success",
                "status": "ok",
                "workers": sorted(parallel_results, key=lambda item: item["worker"]),
            }
        )
        ...
        cleanup = call_tool(
            "remote.bash",
            {**endpoint, "command": f"rm -rf {scratch!r}", "timeout_ms": timeout_ms},
        )
        checks.append(
            {
                "name": "cleanup",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the remote-dev scaffold contract and optional live endpoint behavior."
    )
    ...
    failed = failed or any(
        item.get("status") != "ok" for item in report["local_checks"]
    )
