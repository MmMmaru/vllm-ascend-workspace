...
from core.job_ops import (
    endpoint_from_job_record,
    require_job_id,
    remote_job_status,
    remote_job_stop,
    remote_job_tail,
...
def list_resources() -> list[dict[str, Any]]:
    resources = [
        {
            "uri": "remote://endpoints",
            "name": "Remote endpoints",
...
def _read_artifact_manifest(endpoint_id: str, artifact_id: str) -> str:
    ...
    if text is None:
        raise KeyError(
            f"unknown artifact manifest resource: {endpoint_id}/{artifact_id}"
        )
    return text
...
def read_resource(uri: str) -> dict[str, Any]:
    if uri == "remote://endpoints":
        return {
            ...
            "mimeType": "application/json",
            "text": json.dumps(
                {"endpoints": list_endpoint_records()},
                ensure_ascii=False,
                indent=2,
                sort_keys=True,
            ),
    ...
    if len(parts) == 2 and parts[1] == "jobs":
        return {
            ...
            "mimeType": "application/json",
            "text": json.dumps(
                {"jobs": list_job_records(endpoint_id)},
                ensure_ascii=False,
                indent=2,
                sort_keys=True,
            ),
    ...
    if len(parts) >= 3 and parts[1] == "job":
        ...
        if len(parts) == 4 and parts[3] in {"stdout", "stderr"}:
            return {
                "uri": uri,
                "mimeType": "text/plain",
                "text": _read_job_log(record, parts[3]),
            }
    if len(parts) == 2 and parts[1] == "artifacts":
        ...
        return {
            ...
            "mimeType": "application/json",
            "text": json.dumps(
                {"artifacts_dir": str(directory), "manifests": manifests},
                ensure_ascii=False,
                indent=2,
                sort_keys=True,
            ),
    ...
    if len(parts) == 4 and parts[1] == "artifacts":
        if len(parts) == 4 and parts[3] == "manifest":
            return {
                "uri": uri,
                "mimeType": "application/json",
                "text": _read_artifact_manifest(endpoint_id, parts[2]),
            }
    if len(parts) >= 2 and parts[1] == "jobs-dir":
        return {
            "uri": uri,
            "mimeType": "text/plain",
            "text": str(jobs_dir(endpoint_id)) + "\n",
        }
    raise KeyError(f"unknown resource: {uri}")
...
def call_tool(name: str, arguments: dict[str, Any] | None) -> dict[str, Any]:
    ...
    endpoint = None
    if name not in {"remote.job_status", "remote.job_tail", "remote.job_stop"} or any(
        args.get(k)
    ...
    if name == "remote.read":
        ...
        return remote_read(
            ...
            allow_symlink=bool(args.get("allow_symlink", False)),
            client_context_id=args.get("client_context_id"),
            timeout_ms=timeout_ms,
        )
    ...
    if name == "remote.job_stop":
        return remote_job_stop(
            endpoint, job_id=str(args["job_id"]), force=bool(args.get("force", False))
        )
    if name == "remote.artifact_manifest":
        assert endpoint is not None
        return remote_artifact_manifest(
            endpoint, remote_path=str(args["remote_path"]), timeout_ms=timeout_ms
        )
    ...
    if name == "remote.context_snapshot":
        ...
        return remote_context_snapshot(
            endpoint,
            timeout_ms=timeout_ms,
            live_probe=bool(args.get("live_probe", True)),
        )
