...
class PathPolicyTests(unittest.TestCase):
    def test_join_under_root_accepts_relative_child(self) -> None:
        self.assertEqual(
            join_under_root("/vllm-workspace", "/vllm-workspace/src", "foo.py"),
            "/vllm-workspace/src/foo.py",
        )

    def test_join_under_root_rejects_escape(self) -> None:
        with self.assertRaises(PathPolicyError):
            join_under_root(
                "/vllm-workspace", "/vllm-workspace/src", "../../etc/passwd"
            )

    def test_assert_under_root_allows_root_itself(self) -> None:
        self.assertEqual(
            assert_under_root("/vllm-workspace", "/vllm-workspace"), "/vllm-workspace"
        )
