...
The upstream vLLM community has already implemented the SP (Sequence Parallel) logic. To keep the code simple and maintainable, the downstream SP feature needs to be refactored.
### Does this PR introduce _any_ user-facing change?
Delete flashcomm1.
upstream SP is activated with DP>1, TP>1 and all2allbackend set to allgather_reducescatter
...
```
```text
Historical main flashcomm on (commit/runtime not re-verified in this round)
============ Serving Benchmark Result ============
Successful requests:                     50        
Failed requests:                         0         
Maximum request concurrency:             32        
...
```
```text
Historical main flashcomm off (commit/runtime not re-verified in this round)
============ Serving Benchmark Result ============
Successful requests:                     50        
Failed requests:                         0         
Maximum request concurrency:             32        
...
Qwen3.5-35B-A3B
DP2, TP2, EP
Chat verification: passed (`2 + 2 = 4`, France → Paris, no `!!!`)
...
DP2, TP8, EP (80.5.17.111, 16 logical cards 0-15)
Chat verification: passed (France → Paris, coherent chain of thought; `12 + 30 = 42`, finish_reason=stop, no garbled characters)
