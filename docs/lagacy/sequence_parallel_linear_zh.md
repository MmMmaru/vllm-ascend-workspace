...
- `sequence_parallel_all_gather` 沿第 0 维收集 token shard，并可在计算前
  去掉 padding；
- `sequence_parallel_reduce_scatter` 将第 0 维补齐到 TP world size 的整数倍，
  对各 rank 的部分结果求和，然后重新分发 token shard；
...
如果 Attention metadata 要求在 GEMM 前删除 SP padding，可以在调用
column-parallel forward 时传入 `sequence_parallel_unpadded_size`。
reduce-scatter 则会自动补齐第 0 维，确保 token 数可以被 TP world size 整除。
...
3. 删除对应的模型侧 all-gather、padding、reduce-scatter，以及
   `reduce_results=False` 临时绕行逻辑；
